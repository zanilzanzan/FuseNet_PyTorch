...
class FuseNet(nn.Module):
    def __init__(self, num_labels):
        ...
        classifier = list(models.vgg16(pretrained=True).classifier.children())
...
def CrossEntropy2d():
    def wrap(seg_inputs, class_inputs, seg_targets, class_targets, lambda2, weight=None, pixel_average=True):
        ...
        seg_inputs = seg_inputs.transpose(1, 2).transpose(2, 3).contiguous()
        ...
        # Give lambda values
        lambda1 = 1.0
        lambda2 = np.float(lambda2)
        
        # Combine losses 
        loss = lambda1 * seg_loss + lambda2 * class_loss    
